...
//| echo: false
{
  ...
  // Use fewer angular steps for a snappier feel
  const stepsTheta = 30;
  ...
  const layout = {
    ...
    scene: {
      xaxis: { title: "x", range: [-0.3, Math.PI + 0.3] },
  ...
  // Smooth update: animate with a short transition instead of full redraw
  const update = buildTraces(sweepAngle1);
  await Plotly.animate(div,
    { data: update },
