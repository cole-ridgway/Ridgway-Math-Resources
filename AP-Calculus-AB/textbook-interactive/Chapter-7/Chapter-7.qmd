...
```{ojs}
//| echo: false
viewof sweepAngle1 = Inputs.range([0.05, 2 * Math.PI], {
  value: 0.05,
  step: 0.05,
  label: "Angle of Revolution (radians):"
})
```
```{ojs}
//| echo: false
{
  const Plotly = await require("https://cdn.plot.ly/plotly-2.26.0.min.js");
  const f = x => Math.sqrt(Math.sin(x));
  const a = 0, b = Math.PI;
  const stepsX = 60, stepsTheta = 30;
  ...
  function buildTraces(angle) {
    ...
    return [
      ...
      { x: lineX, y: lineY, z: lineZ, type: "scatter3d",
        mode: "lines", line: { color: "red", width: 5 } }
    ];
  }
  ...
  // Look for an already-rendered div from a previous slider tick
  // and read its current camera so we don't reset the user's rotation
  const existingDiv = document.querySelector("#sol-rev-1");
  const currentCamera = existingDiv?._fullLayout?.scene?.camera ?? defaultCamera;

  const layout = {
    title: { text: "Solid of Revolution: f(x) = √(sin x)", font: { size: 14 } },
    scene: {
  ...
  div.id = "sol-rev-1";
  div.style.cssText = "width:100%; height:480px;";

  await Plotly.react(div, buildTraces(sweepAngle1), layout, { responsive: true });
  return div;
}
```
:::
