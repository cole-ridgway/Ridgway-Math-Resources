...
% --- SECTION 5.1 ---
% \input{preamble.tex} % Assuming your preamble is in a separate file, or paste it above.
...
\begin{objectives}
    ...
    \begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X @{}}
        ...
        \midrule[1.5pt]
        Develop and use properties of the natural logarithmic function & I can use the properties of natural logarithms to rewrite, simplify, and solve expressions. \\
        \midrule
        Understand the definition of the number $e$ & I can explain what the number $e$ represents and how it is defined. \\
        \midrule
...
\begin{definition}[title=Definition of the Natural Logarithmic Function]
    ...
    \]
    The domain is the set of all positive real numbers $(0, \infty)$.
\end{definition}

\textbf{Visualizing the Function:}
Based on the definition as an accumulation function, we can determine:
\begin{itemize}
    \item $\ln(x)$ is \underline{\hspace{2cm}} for $x > 1$.
    \item $\ln(x)$ is \underline{\hspace{2cm}} for $0 < x < 1$.
    \item $\ln(1) = 0$.
\end{itemize}

% Placeholder for student graphing
\begin{figure}[H]
    \centering
    \include{empty graph.tex} 
...
\begin{theorem}[title=Properties of the Natural Logarithmic Function]
    ...
    \begin{enumerate}
        \item \textbf{Domain:} $(0,\infty)$ \quad \textbf{Range:} $(-\infty, \infty)$
...
We can use properties of integrals to derive algebraic properties of logarithms.
...
\tcblower
    ...
    \begin{enumerate}
        \item $\ln(1) = 0$
        \item $\ln(ab) = \ln(a) + \ln(b)$ \hfill (Product Property)
        \item $\ln(a^n) = n \ln(a)$ \hfill (Power Property)
        \item $\ln\left(\frac{a}{b}\right) = \ln(a) - \ln(b)$ \hfill (Quotient Property)
    \end{enumerate}
\end{theorem}

\begin{example}
    Use properties of logarithms to rewrite and simplify the expressions:
    \begin{enumerate}[label=\alph*)]
        \item $\displaystyle \ln \left( \frac{10}{9} \right)$
        \vspace{1in} % Space for student work
        \item $\displaystyle \ln \left( \sqrt{3x+2} \right)$
        \vspace{1in} % Space for student work
        \item $\displaystyle \ln \left( \frac{6x}{5}\right)$
        \vspace{1in} % Space for student work
    \end{enumerate}
...
\begin{exercises}[\textbf{Use properties to expand the expression}]
    \item $\displaystyle \ln \left( \frac{5}{6} \right)$
    \item $\displaystyle \ln \left( \sqrt{2^3} \right)$
    \item $\displaystyle \ln \left( \sqrt[3]{b^2+2} \right)$
    \item $\displaystyle \ln \left( z(z-1)^2 \right)$
\end{exercises}

\newpage
\subsection{The Number $e$}

In algebra, you learned that logarithms require a \textbf{base}. 
\begin{itemize}
    \item Common Log: Base 10 ($\log_{10} x$)
    \item Natural Log: Base $e$ ($\ln x = \log_e x$)
\end{itemize}
...
Once you know that $\ln(e)=1$, you can use logarithmic proeprties to evaluate the natural logarithms of several other numbers. For example, by using the property 
\begin{align*}
    \ln(e^n) &= n \ln(e) \\
    &= n(1) \\
    &= n
...
\begin{example}
    ...
    \setlength{\jot}{0.25in}
    \begin{align*}
     & &&  \text{Write original equation.}\\
     & &&  \text{Take natural log of each side.} \\
     & &&  \text{Logarithmic properties} \\
     & &&  \text{Differentiate.} \\
     & && \\
     & && \text{Solve for $y'$.}\\
     & && \text{Substitute for $y$ and get common denominator.} \\
    &= \frac{(x-2)(x^2+2x+2)}{(x^2+1)^\frac{3}{2}} && \text{Simplify.}
    \end{align*}
\end{example}

\subsubsection*{Derivatives Involving Absolute Value}
Since $\ln(x)$ is only defined for $x>0$, we often work with $\ln|u|$.

The following theorem states that you can differentiate functions of the form $y=\ln|u|$ as if the absolute value sign were not present.
...
Recall that the Power Rule $\int x^n dx$ does not apply when $n=-1$. For this case, we use the Log Rule.
...
\begin{example}
    Integrate $\displaystyle \int \frac{2}{x}dx$
    \vspace{1in}
\end{example}
...
\begin{definition}[title=Guidelines for Integration Strategies]
    \begin{enumerate}
    \item \textbf{Check Basic Formulas:} Is it a simple Power Rule or Trig Rule?
    \item \textbf{Check Log Rule ($u'/u$):} Is the numerator the derivative (or constant multiple) of the denominator?
    \item \textbf{Integration by Substitution:} Find a $u$ such that $du$ is present in the integrand.
    \item \textbf{Algebraic Manipulation:} 
        \begin{itemize}
            \item Trig Identities
            \item Multiply/Divide by a "clever form of 1"
        \end{itemize}
\end{enumerate}
\end{definition}

\begin{example}
    Solve the differential equation $\displaystyle \frac{dy}{dx} = \frac{1}{x\ln(x)}$.
    \vspace{4in}
\end{example}

\subsection{Integrals of Trigonometric Functions}

Previously, we only knew integrals for $\sin x$ and $\cos x$ (and things like $\sec^2 x$). Now, we can find the integrals for $\tan x$, $\cot x$, $\sec x$, and $\csc x$ by rewriting them as rational functions.
...
$$f^{-1}: \{(4,1),(5,2),(6,3),(7,4)\}.$$
Note that of $f$ is equal to the range of $f^{-1}$, and vice versa. The functions $f$ and $f^{-1}$ have the effect of "undoing" each other. That is, when you form the composition of $f$ with $f^{-1}$ or the composition of $f^{-1}$ with $f$. you obtain the identity function.
...
Not every function has an inverse. For $f^{-1}$ to be a function, the original function $f$ must pass the \textbf{Horizontal Line Test}.
...
\subsubsection*{Finding the Inverse Equation}
...
\begin{definition}[title=Guidelines for Finding an Inverse Function]
    \begin{enumerate}
        ...
        \item Verify that $f(f^{-1}(x))$ and $f^{-1}(f(x))=x$.
    \end{enumerate}
\end{definition}

\begin{example}
    Find the inverse function of $\displaystyle f(x)=\sqrt{2x-3}$.
    \vspace{2.5in}
\end{example}
...
Since the graphs are reflections over $y=x$, the slopes are \textbf{reciprocals}.
...
This is the most critical Calculus concept for this section. We can find the slope of the inverse function \textit{without} actually finding the inverse equation.
...
\begin{example}
    Let $\displaystyle f(x)= \frac{1}{4}x^3 + x -1$.
    \begin{enumerate}
        \item What is the value of $f^{-1}(3)$? 
...
Since $f(x) = \ln(x)$ is increasing and one-to-one, it must have an inverse function.
...
The natural exponential function is unique because \textit{it is its own derivative}. It is the solution to the differential equation $y' = y$.
...
\begin{theorem}[title=Derivative of the Natural Exponential Function]
    ...
    \begin{enumerate}
        ...
        \item $\displaystyle \frac{d}{dx}[e^u] = e^u \cdot u' = e^u \frac{du}{dx}$ \quad \textbf{(Chain Rule)}
    \end{enumerate}
\end{theorem}
...
\begin{example}
    ...
    \begin{enumerate}
        \item $\displaystyle 3^x = \frac{1}{81}$
        
        \vspace{1in}
        
        \item $\displaystyle \log_2(x)=-4$
...
\begin{itemize}
    \item When \textbf{differentiating} $a^x$, we \textbf{multiply} by $\ln(a)$.
    \item When \textbf{integrating} $a^x$, we \textbf{divide} by $\ln(a)$.
\end{itemize}
...
\begin{theorem}[title=Integration Rule for Bases Other than $e$]
\tcblower
...
When the Power Rule, $\frac{d}{dx}[x^n] = nx^{n-1}$, was introduced in Chapter 2, the exponent $n$ was required to be a ration number. Now the rule is extended to cover any real value of $n$. 
...
\begin{example}
    ...
    \begin{enumerate}
        ...
        \item $\arcsin(0.3)$ \textit{This requires a calculator in \textbf{radian mode}.} 
...
Even though inverse trig functions are transcendental, their derivatives are \textbf{algebraic} (involving square roots and polynomials).
...
\tcblower
    ...
    \end{align*}
    \textit{Note: The ``co-" functions are just the negatives of their partners.}
\end{theorem}

To derive these formulas, you can use implicit differentiation. For instance, if $y=\arcsin(x)$, then $\sin(y)=x$. Differentiating implicitly yields $\cos(y)y' = 1$.
...
\begin{example}
    \begin{enumerate}
        \item $\displaystyle \frac{d}{dx}[\arcsin(2x)]$
        
        \vspace{1in}
        
        \item $\displaystyle \frac{d}{dx} [\arctan(3x)]$
...
\begin{example}
    ...
    \begin{enumerate}
        ...
        \item $\displaystyle \int \frac{1}{2+9x^2}dx$ 
